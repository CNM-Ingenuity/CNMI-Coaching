...
class Tribe__Events__Community__Tickets__Event_Form {
	...
	public function __construct( $main ) {
		...
		add_filter( 'tribe_events_tickets_woo_display_ecommerce_links', array( $this, 'display_ecommerce_links' ) );
		add_filter( 'tribe_events_tickets_edd_display_ecommerce_links', array( $this, 'display_ecommerce_links' ) );

		add_filter( 'tribe_ticket_filter_attendee_report_link', array( $this, 'modify_report_link' ), 10, 2 );
		add_filter( 'tribe_filter_attendee_order_link', array( $this, 'modify_order_link' ), 10, 2 );
		add_filter( 'tribe_tickets_attendees_event_action_links', array( $this, 'maybe_remove_edit_action_link' ), 10, 2 );
		add_filter( 'tribe_tickets_event_action_links_edit_url', array( $this, 'maybe_force_front_end_edit_post_link' ), 10, 2 );
		add_filter( 'tribe_tickets_venue_action_links_edit_url', array( $this, 'maybe_force_front_end_edit_venue_link' ), 10, 2 );
		add_filter( 'tribe_tickets_plus_woocommerce_order_link_url', array( $this, 'maybe_force_front_end_edit_order_link' ), 10, 2 );

		// use TEC's "Integrations" class for Twenty Seventeen theme to fix front-end Community Tickets layouts
		add_filter( 'tribe_events_twenty_seventeen_remove_sidebar_class', array( $this, 'maybe_remove_twenty_seventeen_sidebar_class' ) );
	}
	 ...
	 * @return boolean
	 */
	public function maybe_remove_twenty_seventeen_sidebar_class() {
		return tribe_community_tickets_is_frontend_attendees_report() || tribe_community_tickets_is_frontend_sales_report();
	}
}